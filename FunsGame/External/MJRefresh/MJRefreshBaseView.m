...
{
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageWithName:MJRefreshSrcName(@"arrow")]];
...
- (void)layoutSubviews
{
    ...
    // 1.箭头
    CGFloat arrowX = self.width * 0.5 - 45;
    //修改by chenli
    if ([self isKindOfClass:[MJRefreshFooterView class]]) {
        self.arrowImage.center = CGPointMake(arrowX, self.height * 0.5);
    }else if ([self isKindOfClass:[MJRefreshHeaderView class]]){
        self.arrowImage.center = CGPointMake(arrowX, self.height * 0.4);
    }
